...
Функция `sorted()` возвращает новый отсортированный список в порядке возрастания, составленный из элементов исходного итерируемого объекта.
...
```
 ...
 - `key=None` - именованный опциональный аргумент, по умолчанию равный *`None`*. Ключ определяет критерий, по которому необходимо производить сравнение между элементами.
...
Воспользуемся функцией для сортировки небольшого набора чисел. Числа хранятся в кортеже. Результатом работы функции будет отсортированный список чисел:
...
Как мы видим, функция возвращает отсортированный в алфавитном порядке список, состоящий из строковых литералов.

Процедура сортировки для списков (`lists`) такая же, но сами списки имеют собственный метод `sort()`. Метод очень похож на рассматриваемую функция, он также может принимать ключ для сортировки, мы можем передать ему параметр `reverse`, но данный метод изменяет список, к которому он был применен. Если ваша задача отсортировать список и передать его дальше, но не изменять его самого, то лучше воспользоваться функцией `sorted()`. 

Чтобы убедиться в том, что метод `sort()` не возвращает новый список, а изменяет тот, с которого он был вызван, рассмотрим пример ниже:
...
[('a', 3), ('b', 2), ('c', 1)]  # Возвращается отсортированный список пар ключ-значение
```

## Сортировка с применением ключей
...
В качестве ключа мы использовали функцию `len()`, которая вызывается для каждого элемента и возвращает критерий для сравнения - длину элемента.
...
> Метод \_\_repr\_\_ позволяет представлять экземпляры в виде ('Dinesh Chugtai', 9.5, 27), a не в виде <\_\_main\_\_.Coder object at 0x10a2d3790>. То есть метод определяет характер репрезентации объекта внутри программы.
...
Мы просто поставили знак минус `-` перед `coder.age`. Ничего сложного. Без параметра `reverse` тоже можно обойтись.
...
В некоторых ситуациях может быть более красиво расписать данную сортировку на две строки, вызывая последовательно функцию сортировки два раза:
...
Чтобы избавиться от лямбда функций следует применять функции модуля `operator`:
