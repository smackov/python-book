...
> Технически многие аннотации типов, показанные ниже, являются излишними, потому что *mypy* может определить их типы по выражениям. Многие из этих примеров имеют две цели: показать как писать аннотации и определять типы.

## Переменные 

Python 3.6 познакомил нас с новым синтаксисом аннотаций переменных в [PEP 526](https://www.python.org/dev/peps/pep-0526), и мы можем пользоваться этим. Рассмотрим примеры:
...
## Более сложные случаи или когда вещи усложняются
...
## Стандартные «утиные типы»

В типичном коде Python многим функциям, которые могут принимать в качестве аргумента список (list) или словарь (dict), требуется, чтобы их аргумент был каким-то образом «наподобие списка» или «наподобие словаря». Конкретное значение словосочетаний «подобный списку» или «подобный словарю» (или что-то подобное) называется «утиными типами».
